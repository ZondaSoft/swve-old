...
@if($edicion == true)
  @if($agregar == true)
      <form method="post" action="{{ url('/convenios/add') }}" enctype="multipart/form-data">
  @else
      <form method="post" action="{{ url('/convenios/edit/'.$legajo->id) }}" enctype="multipart/form-data">
  @endif
@else
  <form method="post" action="{{ url('/convenios/edit/') }}" enctype="multipart/form-data">
@endif


{{ csrf_field() }}
<div class="content-heading" style="height: 67px;max-height: 90px">
   <div class="col-md-7">Convenios colectivos
      <small></small>
   </div>
   ...
   <div class="col-md-4" style="text-align: right;">
       @if($edicion == true)
           ...
           <a href="{{ url('/convenios') }}" class="btn btn-labeled btn-danger mb-2">
              <span class="btn-label"><i class="fa fa-times"></i>
              </span>Cancelar
           </a>
      @else
           <a class="btn btn-oval btn-success" href="/convenios/add" >Agregar</a>
           <a class="btn btn-oval btn-success" href="/convenios/edit/{{ $legajo?$legajo->id:'' }}">Editar</a>
           <a class="btn btn-oval btn-danger" href="/convenios/delete/{{ $legajo?$legajo->id:'' }}">Borrar</a>
      @endif
    </div>
</div>
